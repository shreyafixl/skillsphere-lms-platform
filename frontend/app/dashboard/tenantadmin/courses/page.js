...
export default function CoursesPage() {
  return (
    <TenantAdminLayout topbarTitle="Course Assignments">
      {({ assignments, openAssignCourse, openUploadMaterial, openFilters }) => {
        const totalAssigned = assignments.reduce((s, c) => s + c.assigned, 0)
        const totalCompleted = assignments.reduce((s, c) => s + c.completed, 0)
        ...
        const stats = [
          ...
          },
          {
            title: "Active",
            value: String(
              assignments.filter((c) => c.status === "active").length
            ),
            growth: "4",
            ...
            <PageHeader
              ...
              action={
                <div className="flex flex-wrap gap-2">
                  ...
                  <PrimaryActionButton onClick={openAssignCourse}>
                    <BookOpen size={18} />
                    Assign Course
                  </PrimaryActionButton>
                </div>
