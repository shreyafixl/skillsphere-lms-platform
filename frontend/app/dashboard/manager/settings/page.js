...
export default function ManagerSettingsPage() {
  return (
    <ManagerLayout topbarTitle="Settings">
      {({ managerInfo, openSettings }) => (
        <ManagerPageSection className="space-y-6 sm:space-y-8">
          <PageHeader
            title="Settings"
            description={managerCopy.settingsDescription}
            action={
          ...
          <DashboardCard>
            <SectionHeader
              ...
              subtitle="Your team leadership workspace"
            />
            <dl className="grid gap-4 sm:grid-cols-2">
              {[
                ["Name", managerInfo.admin],
                ["Department", managerInfo.name],
                ["Role", managerInfo.plan],
                ["Team size", `${managerInfo.teamSize} members`],
              ].map(([label, value]) => (
                <div
                  key={label}
