...
export default function UsersPage() {
  ...
  return (
    <DashboardLayout topbarTitle="Users">
      <div className="space-y-6 sm:space-y-8">
        ...
        <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
          <DashboardCard className="xl:col-span-2">
            <SectionHeader title="All Users" subtitle={`${filtered.length} users found`} />
            <div className="mb-5 flex flex-col gap-3 sm:flex-row">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="h-11 w-full rounded-xl border border-slate-200/80 bg-slate-50/80 pl-10 pr-4 text-sm outline-none transition-all focus:border-violet-300 focus:ring-2 focus:ring-violet-500/20 dark:border-slate-700 dark:bg-slate-800/80 dark:text-slate-100 dark:focus:border-violet-500/50"
                />
              </div>
              <button
                type="button"
              ...
              <table className="w-full min-w-[680px] border-collapse">
                <thead>
                  <tr className="border-b border-slate-100 dark:border-slate-800">
                    {["User", "Tenant", "Role", "Status", "Joined", ""].map((col) => (
                      <th key={col || "actions"} className="px-3 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500">
                        {col}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {filtered.map((user) => (
                    <tr key={user.id} className="group border-b border-slate-50 transition-colors last:border-0 hover:bg-violet-50/40 dark:border-slate-800/50 dark:hover:bg-violet-500/10">
                      ...
                      <td className="px-3 py-4 text-sm text-slate-600 dark:text-slate-300">{user.tenant}</td>
                      <td className="px-3 py-4">
                        <span className={cn("rounded-lg px-2.5 py-1 text-xs font-semibold", roleStyles[user.role])}>{user.role}</span>
                      </td>
                      <td className="px-3 py-4">
                        <span className={cn("rounded-full px-2.5 py-1 text-xs font-semibold capitalize", user.status === "active" ? "bg-emerald-50 text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-400" : "bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400")}>
                          {user.status}
                        </span>
                      </td>
                      <td className="px-3 py-4 text-sm text-slate-500 dark:text-slate-400">{user.joined}</td>
                      <td className="px-3 py-4">
                        <button
                          type="button"
                          onClick={() => {
                          ...
                          className="rounded-lg p-2 text-slate-500 hover:bg-white hover:text-violet-600 dark:hover:bg-slate-800 dark:hover:text-violet-400"
                        ...
                        <button
                          ...
                          className="rounded-lg p-2 text-slate-500 hover:text-violet-600 dark:hover:text-violet-400"
                          aria-label="More"
                        >
          ...
          <DashboardCard>
            <SectionHeader title="Recent Registrations" subtitle="Latest sign-ups across tenants" />
            <ul className="space-y-3">
              {recentRegistrations.map((reg, i) => (
                <li
                  key={i}
                  className="flex items-center gap-3 rounded-2xl p-3 transition-colors hover:bg-slate-50 dark:hover:bg-slate-800/60"
                >
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-violet-100 text-sm font-bold text-violet-700 dark:bg-violet-500/20 dark:text-violet-300">
                    {reg.name.split(" ").map((n) => n[0]).join("")}
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-semibold text-slate-800 dark:text-slate-100">{reg.name}</p>
                    <p className="flex items-center gap-1 truncate text-xs text-slate-500 dark:text-slate-400">
                      <Mail size={12} />
                      {reg.email}
                    </p>
                    <p className="text-xs text-violet-600 dark:text-violet-400">{reg.tenant} · {reg.time}</p>
                  </div>
                </li>
              ))}
            </ul>
      ...
      <UserFormModal open={inviteOpen} onOpenChange={setInviteOpen} mode="invite" />
      <UserFormModal
        open={editOpen}
