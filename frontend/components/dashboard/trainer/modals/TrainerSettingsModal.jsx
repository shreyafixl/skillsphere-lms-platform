...
export default function TrainerSettingsModal({ open, onOpenChange, initialName, onSuccess }) {
  ...
  return (
    <DashboardModal open={open} onOpenChange={onOpenChange} title="Trainer Settings" description="Profile and notification preferences.">
      <FormLayout
        loading={loading}
      ...
      >
        ...
        <div className="space-y-3 rounded-2xl border border-slate-100 p-4 dark:border-slate-800">
          ...
          {[
            ["sessionReminders", "Session start reminders"],
            ["submissionAlerts", "New submission alerts"],
          ].map(([key, label]) => (
