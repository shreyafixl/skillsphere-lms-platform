...
export default function ManagerSettingsModal({
  ...
  return (
    <DashboardModal
      ...
      description="Update your profile and team notification preferences."
      size="default"
    >
      <FormLayout
        loading={loading}
      ...
      >
        ...
        <div className="space-y-3 rounded-2xl border border-slate-100 p-4 dark:border-slate-800">
          ...
          {[
            ["emailNotifications", "Email alerts for at-risk learners"],
            ["weeklyDigest", "Weekly team progress digest"],
          ].map(([key, label]) => (
            <label
              key={key}
              className="flex cursor-pointer items-center justify-between gap-3 text-sm text-slate-600 dark:text-slate-300"
            >
              <span>{label}</span>
              <input
                type="checkbox"
